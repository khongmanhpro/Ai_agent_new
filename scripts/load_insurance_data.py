...
async def load_data_with_neo4j():
    ...
    try:
        async with driver.session() as session:
            # Clear existing data trước khi load mới
            print("🧹 Clear dữ liệu cũ...")
            await session.run("MATCH (n) DETACH DELETE n")
            print("✅ Đã clear dữ liệu cũ")
            for customer in SAMPLE_INSURANCE_DATA:
                # Tạo customer node
                await session.run("""
                    CREATE (c:Customer {
                ...
                for policy in customer["policies"]:
                    await session.run("""
                        ...
                        duration=policy.get("duration", 1),  # Default 1 year nếu không có
                        status=policy["status"],
                        vehicle=policy.get("vehicle", ""),
                        plate_number=policy.get("plate_number", ""),
                        monthly_premium=policy.get("monthly_premium", 0),
                        yearly_premium=policy.get("yearly_premium", 0)
                    )
...
def main():
    ...
    # Load config và set environment variables
    import configparser
